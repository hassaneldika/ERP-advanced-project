...
const AddRoleForm = ({ token, setReloadRoles, reloadRoles }) => {
    ...
    // Submission Function
    const AddNewRole = async (userData) => {
        try {
            const response = await axios.post(`${url}api/roles`, userData, {
    ...
    const onSubmit = async (a) => {
        ...
        if (canSubmit) {
            try {
                ...
                const message = await AddNewRole(data);
