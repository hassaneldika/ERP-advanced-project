...
const EditSkillForm = ({ editSkill, token, setReloadSkills, reloadSkills }) => {
  ...
  // Submission Function
  const EditSkill = async (userData) => {
    try {
      const response = await axios.post(`/api/skills/${slug}`, userData, {
  ...
  const onSubmit = async (a) => {
    ...
    if (canSubmit) {
      try {
        ...
        const message = await EditSkill(data);
    ...
    <div className="form-section add-team-form">
      ...
      <section className="form">
        <form onSubmit={onSubmit}>
          ...
          <div className="form-group">
            <input type="submit" className="btn btn-block" value="Edit Skill" />
          </div>
        </form>
      </section>
    </div>
